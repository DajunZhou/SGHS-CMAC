...
global NVAR NG NH MaxItr HMS HMCR PAR PARmin PARmax bwmin bwmax;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
global m nb block_num mns v_std wght u_exp Fi M uc xmin xmax Gu Gv cmac_w input_n output_n Wnum;    %%CMAC参数
input_n = 2; %输入的维数
output_n = 1; %输出的维数
m = 4;  %CMAC的层数
nb = 2;  %每层的块数
...
Gu = ones(input_n,1)*linspace(Gumin,Gumax,block_num);
Gv = ((xmax-xmin)/((block_num-1)*(2*sqrt(-log(uc)))))*ones(input_n,block_num);
...
NVAR= block_num*output_n;         %number of variables 变量数
...
HMS=50;          % harmony memory size 记忆库大小
    ...
    function valcmac = H_CMAC(sol,data_in)
        ...
        for k1 = 1:1:size(data_in,2) %遍历每个样本
            ...
            for i = 1:1:input_n   %遍历输入的每维
                for j = 1:block_num    
                    u_exp(i,j) = exp(-(u(i)-Gu(i,j)).^2/(Gv(i,j).^2));
                end
            end
            for o = 1:output_n
                for j = 1:block_num 
                    wght(o,j) = sol((o-1)*block_num+j);
                end
            end
            Fi = prod(u_exp); 
            output = Fi*wght';
